...
@Component({
  selector: 'app-codigo-dane-list',
  templateUrl: './codigo-dane-list.component.html',
  styleUrls: ['./codigo-dane-list.component.scss']
})
export class CodigoDaneListComponent implements OnInit {
  ...
  lcListItems: CodigoDane[] = [];
  // lcBreadcrumbItems: MenuItem[];
  // lcHome: MenuItem;
  lcConsulta = ''; // input de la consulta
  lcSelectedRow: CodigoDane = null; // Cuando uno se para en un fila de la tabla
  ...
  ngOnInit() {
    ...
    this.eventos.ciudadesBusqueda.subscribe(() => console.log('emitida la busqueda'));
    this.eventos.ciudadCreacion.subscribe((data: any) => {
    ...
    this.eventos.ciudadActualizacion.subscribe((data: any) => {
  ...
  fnBuscar() {
    ...
    this.gService.getAll(this.constant.ciudades, this.lcFiltros)
      .subscribe(
