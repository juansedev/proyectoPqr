...
@Component({
  ...
  templateUrl: './tipos-notificacion-list.component.html',
...
export class TiposNotificacionListComponent implements OnInit {
  ...
  lcListItems: TipoNotificacion[] = [];
  // lcBreadcrumbItems: MenuItem[];
  // lcHome: MenuItem;
  lcConsulta = ''; // input de la consulta
  lcSelectedRow: TipoNotificacion = null; // Cuando uno se para en un fila de la tabla
  ...
  fnPruebaDialog(editing, id) {
    ...
    const ref = this.dialogService.open(TiposNotificacionFormComponent, dialogConfig);
